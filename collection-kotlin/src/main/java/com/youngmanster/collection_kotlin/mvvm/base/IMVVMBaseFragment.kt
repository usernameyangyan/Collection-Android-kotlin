...
/**
 *  author : yangyan
 *  date : 2021/5/10 5:31 PM
 *  description :
 */
abstract class IMVVMBaseFragment<V : ViewDataBinding, VM : BaseViewModel<*>> : ICommonFragment() {
    var mBinding: V? = null
    ...
    private fun initViewDataBinding() {
        mBinding = DataBindingUtil.bind(bindingFragmentLayout!!)
    ...
    protected open fun registerUIChangeLiveDataCallBack() {
        ...
        //关闭界面
        viewModel?.getUcLiveData()?.getFinishEvent()?.observe(this, Observer { activity?.finish() })
        //关闭上一层
        viewModel?.getUcLiveData()?.getOnBackPressedEvent()
