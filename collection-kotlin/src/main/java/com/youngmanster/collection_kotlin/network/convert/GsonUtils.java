...
public class GsonUtils {
    ...
    public static <T> T fromJsonObject(String reader, Class tranformClass) {
        ...
        Type type = new ParameterizedTypeImpl(Config.Companion.getMClASS(), new Class[]{tranformClass});

        if (!TextUtils.isEmpty(Config.Companion.getEXPOSEPARAM())) {
            try {
                JSONObject jsonObject = new JSONObject(reader);
                if (!jsonObject.has(Config.Companion.getEXPOSEPARAM())|| TextUtils.isEmpty(jsonObject.get(Config.Companion.getEXPOSEPARAM()).toString())) {
    ...
    public static <T> T fromJsonArray(String reader, Class listClass) {
        ...
        Type type = new ParameterizedTypeImpl(Config.Companion.getMClASS(), new Type[]{listType});

        if (!TextUtils.isEmpty(Config.Companion.getEXPOSEPARAM())) {
            try {
                JSONObject jsonObject = new JSONObject(reader);
                if (TextUtils.isEmpty(jsonObject.get(Config.Companion.getEXPOSEPARAM()).toString())) {
                    return getGsonExpose().fromJson(reader, type);
                } else {
                    return getGsonWithoutExpose().fromJson(reader, type);
                }
    ...
    private static Gson getGsonExpose() {
        if (gsonExpose == null) {
            ExclusionStrategy excludeStrategy = new SetterExclusionStrategy(Config.Companion.getEXPOSEPARAM());
